...
export default function DashboardAdmin() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex font-sans antialiased">
      ...
      {/* MENÚ LATERAL */}
      <aside className="w-64 bg-slate-900 text-slate-200 p-6 flex flex-col justify-between shadow-xl hidden md:flex">
        <div>
          ...
          <nav className="space-y-1.5">
            {[
              ...
              <button
                key={tab.id}
                onClick={() => { setSeccionActual(tab.id as SeccionModulo); setTiendaSeleccionada(null); }}
        ...
        <button
          ...
          className="w-full bg-slate-800 hover:bg-red-950 text-red-400 hover:text-red-200 font-bold py-3 rounded-xl transition duration-200 text-xs uppercase tracking-wider"
        >
          Cerrar Sesión
        </button>
      </aside>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <header className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-8 pb-5 border-b border-slate-200">
          <div>
            {seccionActual === "fotocopias" && tiendaSeleccionada ? (
              <div className="flex items-center gap-2">
                <button 
                  ...
                  className="bg-white hover:bg-slate-200 border border-slate-300 text-slate-700 px-3 py-1.5 rounded-lg text-xs font-bold transition"
                >
                  Volver a Sucursales
                </button>
                <h1 className="text-2xl font-black text-slate-800">Inventario: {tiendaSeleccionada.nombre}</h1>
              </div>
            ) : (
            ...
            <p className="text-xs text-slate-500 mt-1 font-medium">Sincronización bidireccional activa con la base de datos central</p>
          </div>
          
          <button
            onClick={() => configurarFormulario(seccionActual === "fotocopias" ? (tiendaSeleccionada ? "libro" : "tienda") : "general")}
            className="bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs uppercase tracking-wider px-5 py-3 rounded-xl shadow-md transition duration-200 self-start sm:self-center"
          >
            {seccionActual === "fotocopias" && tiendaSeleccionada ? "Agregar Libro" : "Registrar Nuevo Elemento"}
          </button>
        </header>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 text-xs font-bold uppercase tracking-wider">
                  <th className="p-4">Elemento Registrado</th>
                  <th className="p-4">Detalles y Atributos del Sistema</th>
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700 text-sm">
                ...
                {seccionActual === "fotocopias" && !tiendaSeleccionada && tiendas.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="p-4 text-slate-500">{t.ubicacion_detalle}</td>
                    <td className="p-4 text-center flex justify-center gap-2">
                      <button onClick={() => setTiendaSeleccionada(t)} className="bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold px-3 py-1.5 rounded-lg border border-blue-200 transition">Ver Inventario</button>
                ...
                {seccionActual !== "fotocopias" && centros.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="p-4 text-slate-500 max-w-xs truncate">
                      {seccionActual === "pensiones" && `Precio Promedio: ${c.precio_promedio || "N/D"} | `}
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl border border-slate-200">
            ...
            <form onSubmit={procesarFormulario} className="space-y-4">
              ...
              {tipoFormulario !== "libro" && (
                <>
                  <div>
                    <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Descripción General o Ubicación Detallada</label>
                    <textarea
                      rows={2} required
                      className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-800"
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">URL de Imagen (Enlace de Google Drive)</label>
                    <input
                      type="text" required
                      className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-800"
              ...
              {seccionActual === "pensiones" && tipoFormulario === "general" && (
                <div>
                  <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Costo de Almuerzo Subvencionado o Menú (Ej: Bs. 5.00)</label>
                  <input
                    type="text" required
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-800"
              ...
              {seccionActual === "bibliotecas" && tipoFormulario === "general" && (
                <div>
                  <label className="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Salas de Estudio o Lectura Disponibles (Ej: 3 salas)</label>
                  <input
                    type="text" required
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-slate-800"
                    value={inputMetadata} onChange={(e) => setInputMetadata(e.target.value)}
              ...
              <div className="flex justify-end gap-2 pt-4 border-t border-slate-100">
                <button
                  ...
                  className="bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold px-4 py-2.5 rounded-xl text-xs uppercase tracking-wide transition"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-4 py-2.5 rounded-xl text-xs uppercase tracking-wide transition shadow-md shadow-blue-900/10"
                >
                  Confirmar Guardado
                </button>
              </div>
